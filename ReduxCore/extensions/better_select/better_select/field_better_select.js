...
(function( $ ) {
    ...
    $( document ).ready(
        function() {
            redux.field_objects.select.totaliswp_init();
        }
    );

    redux.field_objects.select.totaliswp_init = function( selector ) {
        if ( !selector ) {
            selector = $( '#redux-form-wrapper' ).find( 'fieldset.redux-container-better_select' );
        }
        $( selector ).each(
            function() {
                var el = $( this );
                ...
                el.find( 'select.redux-better_select-item' ).each(
                    function() {
                        ...
                        var siblings = $( this ).siblings( '.select2_params' );
                        if ( siblings.size() > 0 ) {
                            var select2_params = siblings.val();
    ...
    redux.field_objects.select.addIcon = function( icon ) {
        if ( icon.hasOwnProperty( 'id' ) ) {
        	if ( -1 != icon.text.indexOf('fa-') ) {
	            return "<span class='fontawesome'><i class='" + icon.id + " fa-lg'></i>" + "&nbsp;&nbsp;" + icon.text.replace('fa fa-', 'fa-') + "</span>";
        	} else {
